...
export const generateAutoPitch = (
  ...
  // Determine track based on audience selection (no more random)
  const isInvestor = audience === 'investors' || scrapedData?.audience === 'investors';
  const isAcademic = audience === 'academic';
  const isNonTech = audience === 'nontech';
  const isPeers = audience === 'peers';
